...
subroutine feval(neqn,t,c,f,coef,ipar)
 ...
 !integer, parameter :: nclust = 16
	...
	real(kind(1.d0)) :: t, c(neqn), f(neqn), coef(40), theta(39) ! coef - temperature and 39 evaporation rates 
 	real(kind(1.d0)),save ::  coefs_dHdS(44), sources(2)
    real(kind(1.d0)), save :: dH(22), dS(22)
	integer, save :: I_coll(37), J_coll(37), IJ_evap(37), I_evap(37), J_evap(37), N_evap
	...
	if (ipar(1) .eq. 0) then
		...
		ipar(1) = 1
    call process_input(coefs_dHdS,I_coll,J_coll,IJ_evap,I_evap,J_evap,N_evap, sources)
		call initialize_parameters(theta,coef_quad,coef_lin, dH,dS,I_coll,J_coll,IJ_evap,I_evap,J_evap,N_evap, &
...
!subroutine formation(neqn,c,j_tot,j_by_charge,j_by_cluster,j_all,ipar,coef,theta,dH,dS,I_coll,J_coll,IJ_evap,I_evap,J_evap,N_evap,sources)
!	implicit none
!	integer, parameter :: nclust = 16
...
! real(kind(1.d0)) :: theta(44), dH(22), dS(22)

!	integer :: I_coll(37), J_coll(37), IJ_evap(37), I_evap(37), J_evap(37), N_evap

!	integer, save :: charge(nclust)=0,ind_quad_loss(23,0:34)=0,ind_quad_form(23,0:122)=0

!	integer, save :: ind_quad_loss_extra(23,0:30)=0,ind_quad_form_extra(23,0:72)=0

!	integer, save :: ind_lin_loss(23,0:12)=0,ind_lin_form(23,0:32)=0,fitted(nclust,0:nclust)=0
...
!           & ind_quad_form_extra,source,isconst,fitted,coef,ipar)
...
!	do n = 1, 2*ind_quad_form(22,0)-1, 2 ! loop over all collisions leading out of the system

!		j_add = coef_quad(ind_quad_form(22,n),ind_quad_form(22,n+1),22)*c(ind_quad_form(22,n))*c(ind_quad_form(22,n+1))

!		j_tot = j_tot + j_add

!		j_by_charge(1) = j_by_charge(1) + j_add

!		j_by_cluster(ind_quad_form(22,n)) = j_by_cluster(ind_quad_form(22,n)) + j_add

!		j_by_cluster(ind_quad_form(22,n+1)) = j_by_cluster(ind_quad_form(22,n+1)) + j_add

!		j_all(ind_quad_form(22,n),1) = j_all(ind_quad_form(22,n),1) + j_add

!		j_all(ind_quad_form(22,n+1),1) = j_all(ind_quad_form(22,n+1),1) + j_add
...
subroutine initialize_parameters(theta,coef_quad,coef_lin, dH,dS,I_coll,J_coll,IJ_evap,I_evap,J_evap,N_evap, &
            & sources, ind_quad_loss,ind_quad_form, ind_lin_loss,ind_lin_form,ind_quad_loss_extra,&
            & ind_quad_form_extra,source,isconst,fitted,coef,ipar,K,E,cs)
  ...
  integer, parameter :: nclust=16
	...
	call get_rate_coefs(theta,coef_quad,coef_lin,dH,dS,I_coll,J_coll,IJ_evap,I_evap,J_evap,N_evap,coef,K,E,cs)

end subroutine initialize_parameters

!-----------------------------------------------------------

subroutine get_rate_coefs(theta,coef_quad,coef_lin,dH,dS,I_coll,J_coll,IJ_evap,I_evap,J_evap,N_evap,coef,K,E,cs)
	implicit none
  integer, parameter :: nclust=16
	real(kind(1.d0)) :: coef_quad(nclust,nclust,23),coef_lin(23,23,nclust)
	real(kind(1.d0)) :: theta(39),dH(22),dS(22)
	...
	integer :: I_coll(37), J_coll(37), IJ_evap(37), I_evap(37), J_evap(37), N_evap, i, j
  ...
  temp = coef(1)
	...
	!call get_evap(E,K,dH,dS,I_coll,J_coll,IJ_evap,I_evap,J_evap,N_evap,coef(1))
	
  !  call get_evap(E,temp)
	call get_losses(cs)
...
subroutine get_evap_mcmc(theta,E)
 ...
 !close(22)
	...
	subroutine process_input(coefs_dHdS,I_coll,J_coll,IJ_evap,I_evap,J_evap,N_evap, sources)
		implicit none
		
		integer :: i,j, ncoefs !number of coefficients in the input file, which includes dH, dS of some of the clusters and correlation coefficients for the other values
		real(kind(1.d0)), intent(out) :: coefs_dHdS(44), sources(2)
		integer, intent(out) :: I_coll(37),J_coll(37),IJ_evap(37),I_evap(37),J_evap(37), N_evap! source of monomers: sulph acid and ammonias
		real(kind(1.d0)) :: coef_axil
		integer :: Reason = 0
   ...
   j = 1
		! Reading dH, dS file
		open(10,file='input_dHdS.txt', status='old', action='read')
		DO while (Reason .GE. 0 .AND. j <45)
		 READ(10,*,IOSTAT=Reason) coef_axil
		 coefs_dHdS(j) = coef_axil
      j = j + 1
     ...
     write(*,*) ( coefs_dHdS(j), j=1,44 )

    j = 1
    Reason = 0
		! Reading I_coll
		open(11,file='I_coll.txt')
		DO while (Reason .GE. 0 .AND. j <38)
		 READ(11,*,IOSTAT=Reason) coef_axil
		 I_coll(j) = coef_axil
     j = j + 1 
		END DO
		close(11)
   ...
   write(*,*) 'I_coll'
		write(*,*) ( I_coll(j), j=1,37 )
   j = 1
   Reason = 0
		! Reading J_coll
		open(12,file='J_coll.txt')
		DO while (Reason .GE. 0 .AND. j <38)
		 READ(12,*,IOSTAT=Reason) coef_axil
		 J_coll(j) = coef_axil
      j = j + 1
		END DO
		close(12)
   ...
   Reason = 0
		! Reading IJ_evap
		open(13,file='IJ_evap.txt')
		DO while (Reason .GE. 0 .AND. j <38)
		 READ(13,*,IOSTAT=Reason) coef_axil 
		 IJ_evap(j) = coef_axil
      ...
      write(*,*) 'IJ_evap'
		write(*,*) ( IJ_evap(j), j=1,37 )
		
   j = 1
   Reason = 0
		! Reading I_evap
		open(14,file='I_evap.txt')
		DO while (Reason .GE. 0 .AND. j <38)
		 READ(14,*,IOSTAT=Reason) coef_axil
		 I_evap(j) = coef_axil
      j = j + 1
		END DO
		close(14)
      ...
      write(*,*) 'I_evap'
		write(*,*) ( I_evap(j), j=1,37 )
   j = 1
   Reason = 0
		! Reading J_evap
		open(15,file='J_evap.txt')
		DO while (Reason .GE. 0 .AND. j <38)
		 READ(15,*,IOSTAT=Reason) coef_axil
		 J_evap(j) = coef_axil
      j = j + 1
		END DO
		close(15)
     ...
     ! write(*,*) 'J_evap'
		!write(*,*) ( j_evap(j), j=1,37 )
   
    Reason = 0    
    sources = 0.d0
		! Reading source file
		!open(16,file = 'sources_monomers')
		!READ(16,*,IOSTAT=Reason) coef_axil
    !if(Reason .EQ. 0) then
     ! sources(1) = coef_axil
    !else
     ! write(*,*) 'Failed reading monomer source'
    !end if
